...
這是一套可在自己電腦上執行、也可部署成公開展示網站的台股研究工具。它會整理官方公開資料、具來源標示的 Yahoo／FinMind 備援資料、估值情境、財務風險訊號與新聞索引，也能讓使用者自行下載 PDF。
...
- 優先讀取 TWSE、TPEx 與公開資訊觀測站資料；月營收以最新 OpenAPI 搭配 MOPS 官方歷史封存補足趨勢。雲端主機若遭官方端點暫時拒絕，才以 FinMind 的結構化月營收／季度 EPS 備援，並在畫面標出來源、日期、`fallback` 與 stale 狀態。
- 把「已確認資料」和「模型推估」分開顯示。
...
| 月營收、財報 | TWSE／TPEx OpenAPI、公開資訊觀測站 | 官方端點暫時不可用時，月營收與季度 EPS 可使用 FinMind 結構化備援；每筆保留 `fallback`、來源與核對官方揭露的提示 |
