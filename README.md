...
1. 五個分析步驟完成後，網頁立即顯示可閱讀的分析結果。
2. PDF 報告在背景逐章產生，畫面另列章節與寫檔進度。
3. 寫檔完成後，同時顯示實際檔名、`output` 儲存狀態與兩個下載入口。
4. SSE 暫時斷線或頁面重新整理時，任務狀態與事件可恢復；完成後才清除 session 任務指標。
5. 若只有 PDF 失敗，已取得的分析結果會保留並提供重新執行操作。
