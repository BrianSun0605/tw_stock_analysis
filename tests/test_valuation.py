...
def _eps(year, quarter, value=1.0):
    return {"year": year, "quarter": quarter, "eps": value, "label": f"Q{quarter} {year}"}
...
def test_eps_growth_rejects_quarter_gaps():
    data = [_eps(2023, q) for q in range(1, 5)] + [
        _eps(2024, 1), _eps(2024, 2), _eps(2024, 4), _eps(2025, 1)
    ]
...
def test_piotroski_does_not_award_missing_history():
    analyzer = _analyzer(price_info={"currentPrice": 100, "returnOnAssets": 0.12, "totalCash": 10})
