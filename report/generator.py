...
class PDFReport:
    def __init__(self, stock_info: Dict[str, Any], price_data: Dict[str, Any], price_info: Dict[str, Any],
                 revenue_data: List[Dict[str, Any]], revenue_chart: Optional[str],
                 eps_data: List[Dict[str, Any]], eps_chart: Optional[str],
    ...
    def generate(self, filename=None):
        if filename is None:
            ...
            filename = f"{stock_id}_report_{ts}.pdf"
    ...
    def _render_table(self, col_widths, items, font_size=9, row_height=7):
        for i, (lab, val) in enumerate(items):
            fill = i % 2 == 0
            self.pdf.set_fill_color(245, 250, 255) if fill else self.pdf.set_fill_color(255, 255, 255)
            self._font("", font_size)
            self.pdf.cell(col_widths[0], row_height, f"  {lab}", border=1, fill=True)
            self.pdf.cell(col_widths[1], row_height, val, border=1, fill=True, align="C")
            self.pdf.ln()
    ...
    def _add_revenue_section(self):
        ...
        if self.revenue_data:
            ...
            latest_yoy_values = [r["yoy"] for r in self.revenue_data if r.get("yoy") is not None]
            if latest_yoy_values:
                avg_yoy = sum(latest_yoy_values) / len(latest_yoy_values)
                recent_yoy = latest_yoy_values[-3:] if len(latest_yoy_values) >= 3 else latest_yoy_values
                avg_recent_yoy = sum(recent_yoy) / len(recent_yoy)
                self._font("", 10)
                trend_parts = []
                if avg_yoy > 5:
                    trend_parts.append(f"整體營收年增率平均約 {avg_yoy:.1f}%，營收趨勢偏正向。")
                elif avg_yoy < -5:
                    trend_parts.append(f"整體營收年增率平均約 {avg_yoy:.1f}%，營收呈現衰退。")
                else:
                    trend_parts.append(f"整體營收年增率平均約 {avg_yoy:.1f}%，營收大致持平。")
                if abs(avg_recent_yoy - avg_yoy) > 10:
                    if avg_recent_yoy > avg_yoy:
                        trend_parts.append(f"近 3 期年增率上升至 {avg_recent_yoy:.1f}%，動能增強。")
                    else:
                        trend_parts.append(f"近 3 期年增率降至 {avg_recent_yoy:.1f}%，動能放緩。")
                pi = self.price_info if isinstance(self.price_info, dict) else {}
    ...
    def _add_eps_section(self):
        ...
        if self.eps_data:
            ...
            if len(eps_values) >= 2:
                ...
                if len(recent_eps) >= 2:
                    trend_text = ""
                    if all(recent_eps[i] <= recent_eps[i+1] for i in range(len(recent_eps)-1)):
                        trend_text = "近幾季 EPS 呈現上升趨勢，獲利能力持續改善。"
                    elif all(recent_eps[i] >= recent_eps[i+1] for i in range(len(recent_eps)-1)):
        ...
        try:
            ...
            if va.get("is_etf"):
                ...
                if info.get("nav_price"):
                    ...
                    if price:
                        prem = round((price - info['nav_price']) / info['nav_price'] * 100, 2)
                        sign = "+" if prem > 0 else ""
                        color = "溢價" if prem > 1 else ("折價" if prem < -1 else "平價")
                        items.append(("折溢價", f"{sign}{prem}% ({color})"))
                if info.get("expense_ratio"):
                    er = info["expense_ratio"]
                    er_color = "偏低" if er < 0.005 else ("合理" if er < 0.01 else "偏高")
                    items.append(("費用率", f"{er*100:.3f}% ({er_color})"))
                if info.get("total_assets"):
                    items.append(("資產規模 AUM", self._format_aum(info["total_assets"])))
                if info.get("avg_volume"):
    ...
    def _add_peers_section(self):
        ...
        if is_etf:
            ...
            for p in peers:
                ...
                yield_val = f"{p['dividend_yield']:.2f}%" if p.get("dividend_yield") is not None else "—"
                label = p.get("name", "—")
                self.pdf.cell(col_w[0], 7, label, border=1, align="C")
                self.pdf.cell(col_w[1], 7, price, border=1, align="C")
    ...
    def _add_glossary(self):
        ...
        if is_etf:
            entries = [
                ("NAV (淨資產價值)", "ETF 每股對應的實際資產價值，計算方式為（基金總資產－總負債）÷ 發行股數。市價偏離 NAV 即產生折溢價。"),
